...
function mapToController( controller, root, object, model, value, arrayCreation ){
	...
	for(var key in model){
		...
		if( _.isString( n ) ){
			...
			(function createHelpers( _key ){
				controller['fill' + capName ] = function( _as, keepContent ) {
					m.startComputation();
					if( !keepContent )
						object[ _key ].splice( 0, object[ _key ].length );
					_as.forEach( function( _v ){
						if( _v ){
							object[ _key ].push( mapToController( controller, root, {}, n[0], _v, arrayCreation ) );
						}
					} );
					m.endComputation();
				};
				controller['addTo' + capName ] = function( _v, unique ) {
					m.startComputation();
					if( _v ){
						if( unique )
							controller['removeFrom' + capName ]( _v );
						object[ _key ].push( mapToController( controller, root, {}, n[0], _v, arrayCreation ) );
					}
					m.endComputation();
				};
				controller['removeFrom' + capName ] = function( obj ) {
					m.startComputation();
					if( _.isNumber( obj ) )
						object[ _key ].splice( obj, 1);
					else{
						var index = -1;
						object[ _key ].forEach( function( elmnt, idx ){
							if( elmnt() === obj )
								index = idx;
						} );
						if( index > -1 )
							object[ _key ].splice( index, 1);
					}
					m.endComputation();
				};
				controller['hasIn' + capName ] = function( obj ) {
					var index = -1;
					object[ _key ].forEach( function( elmnt, idx ){
						if( elmnt() === obj )
							index = idx;
