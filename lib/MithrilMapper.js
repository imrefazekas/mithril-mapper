	...
	let prop = function () {
		if (arguments.length) {
			if ( parseFn ) embeddedProp( parseFn( arguments[0] ) )
				...
				let mDate = moment( arguments[0], ctrl.dateFormat || defaultFormat )
				if ( mDate.isValid() )
					embeddedProp( mDate.valueOf() )
			}
		}
		let value = embeddedProp()
...
function mapToController ( controller, root, object, model, value, arrayCreation ) {
	...
	for (let key in model) {
		...
		if ( _.isString( n ) ) {
			...
			let createHelpers = function ( CTRL, CAPNAME, OBJ, ROOT, KEY, M, AC ) {
			...
			createHelpers( controller, capitalizeFirstLetter( key ), object, root, key, n, arrayCreation )
		}
	...
	updateModel: function (name, C, M, O) {
		...
		function updateFn ( origin, model, object ) {
			...
			if ( _.isFunction( model ) ) {
				...
				let originalItem = origin[0]
				model.splice( 0, model.length )
				object.forEach( function ( item ) {
					let viewObject = mapToController( C, M, {}, originalItem || item, null, false )
